...
if __name__ == "__main__":
    ...
    for ii in range(85):
        ks = [ktwenty[ii], kfifth[ii], ktwofifth[ii], ksolar[ii], ktwice[ii]]

    kgrid = (interpolate.bisplrep(Teff,logg,ktarget, kx=1, ky=1))
    alphagrid = (interpolate.bisplrep(Teff,logg,alphatarget, kx=1, ky=1))
    Teffnew,loggnew = np.mgrid[10000:80000:200j, 2:4.5:200j]
    ...
    for ix in range(np.shape(knew)[0]):
        for iy in range(np.shape(knew)[1]):
            if knew[ix,iy] > 1:
                knew[ix,iy] = 1
            if knew[ix,iy] < 0:
                knew[ix,iy] = 0
    for ix in range(np.shape(alphanew)[0]):
        for iy in range(np.shape(alphanew)[1]):
            if alphanew[ix,iy] > .7:
                alphanew[ix,iy] = .7
            if alphanew[ix,iy] < .55:
                alphanew[ix,iy] = .55
    ...
    ax.set_ylabel('Log[g]',fontsize=16,fontweight="bold")
    ax.set_xlabel('Teff',fontsize=16,fontweight="bold")
    ...
    ax.set_ylabel('Log[g]',fontsize=16,fontweight="bold")
    ax.set_xlabel('Teff',fontsize=16,fontweight="bold")
    ax.set_zlabel(r'$\alpha$',fontsize=16,fontweight="bold")
    ax.set_xlim(10000,80000)
    #ax.set_ylim(4.5, 2)
    ax.set_zlim(.55,.7)
